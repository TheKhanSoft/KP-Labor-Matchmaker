...
<head>
    ...
    <title>{{ $title ?? 'KP Labor Matchmaker - Employer Workspace' }}</title>
...
<body class="min-h-full flex flex-col font-sans relative overflow-x-hidden bg-slate-950 text-slate-100" 
      x-data="{ 
          ...
          searchResults: [],
          allItems: [
              { name: 'Search Worker Directory', url: '/directory', desc: 'Browse and unlock registered skilled workers registry', tags: 'directory workers search register trades' },
              { name: 'Buy Token Credits', url: '/credits', desc: 'Purchase token credits using mobile payment simulation', tags: 'credits token topup wallet purchase buy easypaisa' },
              { name: 'Order & Billing History', url: '/orders', desc: 'Review your credit purchase transactions and status', tags: 'orders billing transactions invoice history logs' },
              { name: 'Active Jobs Board', url: '/jobs', desc: 'View active labor job postings', tags: 'jobs posting active employer work contractor' },
              { name: 'User Guide', url: '/guide', desc: 'Step-by-step documentation for hiring and payments', tags: 'guide instructions faq help tutorial' },
              { name: 'Helpline Support', url: '/contact', desc: 'Get in touch with customer support and assistance', tags: 'contact help support phone email location' }
          ],
          init() {
    ...
    @if (\App\Models\Setting::get('enable_maintenance_banner', false))
        <div class="bg-amber-600 text-amber-50 px-4 py-2 text-center text-xs font-bold font-sans flex items-center justify-center gap-2 border-b border-amber-500/30 z-50 relative">
            <svg class="w-4 h-4 text-amber-550 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex flex-col lg:flex-row h-full min-h-screen p-4 sm:p-6 lg:p-8 gap-6 w-full max-w-[1680px] mx-auto z-10 relative">
        ...
        <aside class="hidden lg:flex flex-col w-72 shrink-0 sticky top-8 h-[calc(100vh-4rem)] border border-slate-800 bg-slate-900/80 dark:bg-slate-950/40 backdrop-blur-xl rounded-3xl shadow-xl p-6 transition-all duration-300 overflow-hidden">
            <div class="pb-6 mb-6 border-b border-slate-800 shrink-0">
                <a href="/directory" class="flex items-center gap-3.5 group">
                    ...
                    <div>
                        <span class="font-extrabold tracking-tight text-sm block text-slate-100 group-hover:text-indigo-400 transition-colors duration-300">
                            {{ \App\Models\Setting::get('site_short_name', 'KP-LM') }} Employer
                        </span>
                        <span class="text-[9px] text-indigo-500 font-bold tracking-widest uppercase block -mt-0.5">Workspace</span>
                    </div>
                </a>
            </div>
            ...
            <nav class="flex-grow space-y-1.5 overflow-y-auto pr-1">
                @php
                    $navItems = [
                        ['url' => '/directory', 'label' => 'Search Directory', 'icon' => '🔍', 'exact' => true],
                        ['url' => '/purchase', 'label' => 'Buy Credits', 'icon' => '🪙', 'exact' => true],
                        ['url' => '/orders', 'label' => 'Order History', 'icon' => '📋', 'exact' => true],
                        ['url' => '/credits', 'label' => 'Credits Ledger', 'icon' => '📝', 'exact' => true],
                        ['url' => '/jobs', 'label' => 'Job Board', 'icon' => '💼', 'exact' => false],
                        ['url' => '/guide', 'label' => 'User Guide', 'icon' => '📖', 'exact' => false],
                    ];
                @endphp
                ...
                <div class="mt-auto p-4 border border-slate-800/80 bg-slate-850/60 rounded-2xl flex flex-col gap-3 shadow-inner">
                    <div class="flex items-center gap-3.5">
                        ...
                        <div class="flex-grow min-w-0">
                            ...
                            <span class="block text-[9px] font-bold text-slate-550 truncate mt-0.5">{{ Auth::user()->email }}</span>
                        </div>
                    </div>
                    
                    <div class="p-2.5 bg-slate-900 rounded-xl text-center border border-slate-800 text-[10px] font-bold font-mono text-indigo-400">
                        🪙 Balance: {{ Auth::user()->available_credits }} Credits
                    </div>
        ...
        <div x-show="mobileSidebarOpen" class="relative z-50 lg:hidden" role="dialog" aria-modal="true" style="display: none;">
            ...
            <div class="fixed inset-y-0 left-0 flex w-full max-w-xs bg-slate-900 border-r border-slate-800 p-6 flex-col">
                <div class="p-4 mb-6 border-b border-slate-800 flex justify-between items-center">
                    <a href="/directory" class="flex items-center gap-2.5">
                        <div class="h-8 w-8 rounded-lg bg-indigo-600 flex items-center justify-center text-white font-extrabold text-sm shadow-md">
                            {{ substr(\App\Models\Setting::get('logo_text', 'KP'), 0, 2) }}
                        </div>
                        <span class="font-extrabold text-sm text-slate-100 dark:text-white">{{ \App\Models\Setting::get('site_short_name', 'KP-LM') }} Employer</span>
                    </a>
                ...
                @auth
                    <div class="p-4 border border-slate-800 bg-slate-855 m-4 rounded-2xl flex flex-col gap-3">
                        <div class="flex items-center gap-3">
                            <div class="h-8 w-8 rounded-full bg-indigo-500/15 flex items-center justify-center text-indigo-650 dark:text-indigo-400 font-extrabold text-xs">
                                {{ substr(Auth::user()->name, 0, 1) }}
                            </div>
                            <div class="flex-grow min-w-0">
                        ...
                        <div class="p-2 bg-slate-950 border border-slate-800 rounded-lg text-center font-mono text-[9px] font-bold text-indigo-400">
                            🔑 Balance: {{ Auth::user()->available_credits }} Credits
                        </div>
            ...
            <header class="border-b border-slate-800 bg-slate-900/60 dark:bg-slate-950/40 backdrop-blur-md sticky top-0 z-40 py-4 px-6 lg:px-8">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <button type="button" @click="mobileSidebarOpen = true" class="lg:hidden p-2 rounded-xl border border-slate-800 text-slate-400 hover:text-slate-100 hover:bg-slate-855">
                        ...
                        <h2 class="text-xs sm:text-sm font-extrabold text-slate-400 capitalize tracking-tight flex items-center gap-2">
                            <span class="p-1.5 rounded bg-indigo-500/10 text-indigo-500 text-xs">💼</span>
                            <span>Employer Workspace</span>
                            <span class="text-slate-650">/</span>
                            <span class="text-indigo-500 font-semibold font-mono tracking-wide">{{ Route::current() ? str_replace('admin.', '', Route::currentRouteName()) : 'workspace' }}</span>
                        </h2>
                    </div>
        ...
        <div x-show="searchOpen" 
            ...
            <div class="flex items-center gap-3 border-b border-slate-800 px-4 py-3 bg-slate-950/20">
                ...
                <input x-ref="searchInput" 
                       x-model="searchQuery" 
                       type="text" 
                       placeholder="Search workspace sections or guides..." 
